...
import { X, Save, BookOpen, CalendarX } from 'lucide-react';
  ...
  const handleSave = async () => {
    ...
    try {
      await onSave({
        ...
        difficulty,
        nextCourseDate: nextCourseDate, // Pass empty string to clear the date, undefined to keep unchanged
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                {nextCourseDate && (
                  <button
                    type="button"
                    onClick={() => setNextCourseDate('')}
                    className="px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-500 dark:text-gray-400 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 dark:hover:text-red-400 hover:border-red-300 dark:hover:border-red-700 transition-all"
                    title={t.adminClearDate || 'Clear date'}
                  >
                    <CalendarX size={18} />
                  </button>
                )}
